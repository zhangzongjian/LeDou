	...
	public void 扫荡() {
		try {
			...
			if(doc1.text().contains("挑战次数不足")){
				...
				String result = doc1.text().substring(doc1.text().lastIndexOf("获得了"),doc1.text().lastIndexOf("。"));
		...
		try {
			...
			while(true) {
				...
				//打死最后一个boss
					...
					String result = doc1.text().substring(doc1.text().lastIndexOf("获得了"),doc1.text().indexOf("查看乐斗过程"));
					message.put("挑战情况", result);
					return;
				}
				//挑战失败，不复活
