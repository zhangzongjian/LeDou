...
public class 竞技场 extends 乐斗项目 {
	...
	public void 挑战() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(userKey, mainDoc, "竞技场");
			if (doc.text().contains("赛季中")) {
				int num = 0;
				while (!doc.text().contains("免费挑战次数已用完")) {
					if(doc.text().contains("开始挑战")) {
						doc = DocUtil.clickTextUrl(userKey, doc, "开始挑战");
					} else {
						doc = DocUtil.clickTextUrl(userKey, doc, "免费挑战");
					}
				...
				if (DocUtil.isHref(doc, "领取奖励")) {
					DocUtil.clickTextUrl(userKey, doc, "领取奖励");
