	...
	public void xuYuan(){
		try {
			...
			if(doc.text().contains("领取许愿奖励")) {
				...
				message.put("每日许愿奖励", DocUtil.substring(doc.text(), "【每日许愿】", 6, "。"));
			}
			if(doc.text().contains("首胜后才能许愿")) {
				message.put("许愿情况", "首胜后才能许愿！");
				return;
			}
			DocUtil.clickTextUrl(DocUtil.clickURL(doc.getElementsByAttributeValueMatching("href", "sub=4").get(0).attr("href")),"向月敏上香许愿");
