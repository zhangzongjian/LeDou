...
public class 活动集合 extends 乐斗项目 {
	...
	public void 大宝树() {
		try {
			...
			if(浇水.text().contains("7颗果实")) {
				Document 收获 = DocUtil.clickTextUrl(doc, "收获");
	...
	public void 大笨钟() {
		try {
			...
			message.put("活动9", "【大笨钟】");
			Document doc = DocUtil.clickTextUrl(mainDoc, "大笨钟");
	...
	public void 九宫宝库() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(mainDoc, "九宫宝库");
			String type = doc.text();
			int num = 0;
			while(true) {
				//进入时，界面为转盘
				if(type.contains("转动轮盘")) {
					doc = DocUtil.clickTextUrl(doc, "转动轮盘");
					if(doc.text().contains("开始寻宝")) {
						doc = DocUtil.clickTextUrl(doc, "开始寻宝");
						Random random = new Random();
						//开三次石棺
						for(int i=0; i<3; i++) {
							doc = DocUtil.clickTextUrl(doc, "石棺", random.nextInt(9-i));
						...
						doc = DocUtil.clickTextUrl(doc, "退出宝库");
					}
				...
				//进入时，界面为石棺
					...
					for(int i=0; i<3-a; i++) {
						doc = DocUtil.clickTextUrl(doc, "石棺", random.nextInt(9-a));
						if(doc.text().contains("抽奖卡不足")) {
							message.put(num+"石棺抽奖"+i, "抽奖卡不足!");
							return;
						}
	...
	public void 登录商店() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(mainDoc, "登录商店");
			int i = 0;
			while(true) {
				doc = DocUtil.clickTextUrl(doc, "兑换", 2);
	...
	public void 摇摇乐() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(mainDoc, "摇摇乐");
			int i = 0;
			while (true) {
				doc = DocUtil.clickTextUrl(doc, "转动");
	...
	public void 做任务领斗币() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(mainDoc, "做任务领斗币");
			int i = 0;
			while (doc.toString().contains("act_task")) {
				doc = DocUtil.clickURL(doc.getElementsByAttributeValueMatching("href", "act_task").attr("href"));
	...
	public void 好礼步步升() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(mainDoc, "好礼步步升");
			doc = DocUtil.clickTextUrl(doc, "领取");
			if(doc != null)
	...
	public void 登录有礼() {
		try {
			...
			Document doc = DocUtil.clickTextUrl(mainDoc, "登录有礼");
			doc = DocUtil.clickTextUrl(doc, "领取",-1);
			if(doc != null)
				message.put("登录有礼领取",
