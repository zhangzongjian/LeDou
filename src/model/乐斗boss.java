...
public class 乐斗boss {
	...
	public void 一键挑战() throws IOException, SocketTimeoutException {
		...
		Elements elements = doc.getElementsByAttributeValueMatching("href",
				"3038");
		Document temp = Jsoup.parse(MyUtil.substring(doc.toString(), "侠：", 2,
				"1："));
		...
		for (int i = 0; i < size; i++) {
			...
			if (d.text().contains("侠侣") && d.text().contains("查看乐斗过程")) {
				message.put("乐斗结果" + i,
						MyUtil.substring(d.text(), "侠侣", 2, "查看乐斗过程"));
			} else {
		...
		if (doc1.text().contains("尚未加入任何帮派")) {
			...
			Document temp1 = Jsoup.parse(MyUtil.substring(doc1.toString(),
					"侠：", 2, "帮："));
			Elements boss1 = temp1.getElementsContainingOwnText("乐斗");
			boss1.removeAll(temp1.getElementsByAttributeValueMatching("href",
					"totalinfo"));
